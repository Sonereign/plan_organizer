...
def detect_date_columns(df):
    ...
    try:
        logger.debug("Detecting date columns.")
        date_cols = [col for col in df.columns if is_date(col)]
        if date_cols:
            logger.debug(f"Date columns detected: {date_cols}")
...
def format_date_columns(df, first_date_col, last_date_col):
    """Format date columns from first to last to 'Mon 14/9/YYYY'."""
    try:
        if first_date_col and last_date_col:
            ...
            df.rename(columns={col: pd.to_datetime(col, dayfirst=True).strftime("%a %d/%m/%Y") for col in date_range}, inplace=True)
...
def split_sections(df):
    ...
    try:
        ...
        return pd.concat([houses, add_empty_separator(df), youth_hostel, add_empty_separator(df), the_rest], ignore_index=True)
    except Exception as e:
...
def per_zone_per_type_stage1(input_file, output_file):
    """
    Process the input file (availabilityPerZone) and save the result to the output file.
    """
    logger.info("#######################################################")
    logger.info(f"Running Stage 1 with input file: {input_file}")
    ...
    try:
        ...
        logger.info(f"Stage 1 completed. File saved as {output_file}")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    INPUT_FILE = "sources/availabilityPerZone2025.xls"
    OUTPUT_FILE = "zone_stage1_output.xlsx"

    # Run stage1
    per_zone_per_type_stage1(INPUT_FILE, OUTPUT_FILE)
